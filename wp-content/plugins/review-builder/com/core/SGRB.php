...
class SGRB
{
				...
				foreach ($arr as $ar) {
					if (!@empty($postMetaValue['post_review'])) {
						//if ($ar->getId() == $postMetaValue['post_review'][0]) {
							$reviews = SGRB_ReviewModel::finder()->findByPk($postMetaValue['post_review'][0]);
							if ($reviews) {
								$reviewsArray[] = $reviews;
							}
						//}
	...
	public function mediaButtonThickboxs()
	{
		...
		<div id="sgrb-thickbox" title="Insert review" style="height:0px;width:350px;display:none">
			<div class="wrap">
				<p class="insert-title">Insert the shortcode to show a Review.</p>
				<?php if (SGRB_PRO_VERSION) :?>
				<div class="sgrb-widget-review-insert-type">
					<select id="sgrb-type-buttons-id">
						<option value="1">Insert Review</option>
						<option value="2">Insert Widget of Reivew</option>
					</select>
				</div>
				<?php endif ;?>
				...
				<div class="sgrb-insert-widget" style="display: none;">
					...
					<input type="text" class="sgrb-widget-link" style="width:100%" placeholder="you review page url...">
				</div>
				<?php endif ;?>
