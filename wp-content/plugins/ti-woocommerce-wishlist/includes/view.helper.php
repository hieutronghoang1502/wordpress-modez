...
class TInvWL_View {
	...
	private static function init() {
		add_filter( 'tinvwl_view_header', array( __CLASS__, 'header' ) );
		if ( ! empty( self::$_redirect ) ) {
			return add_filter( 'tinvwl_view_header', array( __CLASS__, '_redirect' ), 5 );
		}
		$message = self::get_session_arr( '_errors' );
		if ( ! empty( $message ) ) {
			add_filter( 'tinvwl_view_header', array( __CLASS__, '_error' ), 20 );
		}
		$message = self::get_session_arr( '_tips' );
		if ( ! empty( $message ) ) {
			add_filter( 'tinvwl_view_header', array( __CLASS__, '_tip' ), 30 );
		}
		$message = self::get_session_arr( '_attentions' );
		if ( ! empty( $message ) ) {
			add_filter( 'tinvwl_view_header', array( __CLASS__, '_attention' ), 40 );
		}
	}
