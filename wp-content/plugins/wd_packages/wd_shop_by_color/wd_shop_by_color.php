...
if (!class_exists('WD_Shopbycolor')) {
	class WD_Shopbycolor {
		...
		public function wd_enqueue_color_picker(  ) {
			...
			wp_enqueue_script( 'wd-product.color.js', PC_JS.'/product_color.js',array( 'wp-color-picker','jquery' ));
			wp_enqueue_style( 'wd-product.color.css', PC_CSS.'/product_color.css');				
		}	
...
if (!class_exists('WD_Widget_PC_Color_Nav')) {
	class WD_Widget_PC_Color_Nav extends WP_Widget {
		...
		function __construct() {
			...
			$this->wpdance_widget_description	= __( 'Shows a color attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'wpdancelaparis' );
			$this->wpdance_widget_idbase 		= 'wpdance_color_nav';
			$this->wpdance_widget_name 			= __( 'WD - Color Layered Nav', 'wpdancelaparis' );
