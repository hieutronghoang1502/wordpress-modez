...
class SIP_Reviews_Shortcode_WC_Admin {
    ...
    add_action( 'admin_notices', array( &$this,  'sip_rswc_admin_notice') );
    add_action( 'admin_init', array( &$this,  'sip_rswc_nag_ignore') );
  ...
  public function sip_rswc_action_links( $links ) {
    ...
    if( !class_exists( 'SIP_Reviews_Shortcode_WC_Pro' ) ) {
      $plugin_links[] = '<a target="_blank" href="' .SIP_RSWC_PLUGIN_PURCHASE_URL. '?utm_source=wordpress.org&utm_medium=SIP-panel&utm_content=v'. SIP_RSWC_VERSION .'&utm_campaign='.SIP_RSWC_UTM_CAMPAIGN.'" style="color: red; font-weight: bold">' . __( 'Go Pro!', 'sip-reviews-shortcode' ) . '</a>';
    }

    $plugin_links[] = '<a href="' . admin_url( 'admin.php?page=sip-reviews-shortcode-settings' ) . '">' . __( 'Settings', 'sip-reviews-Shortcode' ) . '</a>';
   ...
   * @since 1.0.1
   */
  public function sip_rswc_admin_notice() {
    global $current_user ;
    $user_id = $current_user->ID;
    /* Check that the user hasn't already clicked to ignore the message */
    if( ! class_exists( 'SIP_Reviews_Shortcode_Pro' ) ) {
      if ( !get_user_meta($user_id, 'sip_rswc_ignore_notice' ) ) { ?>
        
        <div class="notice">
          <div style="clear:both;"></div>

          <table style="width:98.5%; margin-top:10px; border-spacing:0px;">
            <tr class="sip-notification-message">
              <td>
                <div class="icon">
                  <img title="" src="<?php echo SIP_RSWC_URL . "admin/assets/images/icon-reviews-shortcode.png" ?>" alt="" />
                </div>
              </td>
              <td>
                <div class="title">Do you want even more <b>Features</b>?<br /></div>
                <div class="subbtitle">Extend standard plugin functionality with reviews submission shortcode, aggregated reviews from different products, language support and extra display styles. </div>
              </td>
              <td>
                <div class="button_div">
                  <a class="button" target="_blank" href="https://shopitpress.com/plugins/<?php echo SIP_RSWC_PLUGIN_SLUG ; ?>/?utm_source=wordpress.org&amp;utm_medium=banner&amp;utm_content=v<?php echo SIP_RSWC_VERSION; ?>&amp;utm_campaign=<?php echo SIP_RSWC_UTM_CAMPAIGN ; ?>"><?php _e( 'Learn More', 'sip-reviews-shortcode' ); ?></a>
                  <p style="text-align:center;"><span class="dismiss"><?php printf(__('<a href="%1$s">Dismiss</a>'), 'admin.php?page=sip-reviews-shortcode-settings&sip_rswc_nag_ignore=0'); ?></span></p>
                </div>
              </td>
            </tr>
          </table>
   ...
   * @since 1.0.1
   */
  public function sip_rswc_nag_ignore() {
    global $current_user;
    $user_id = $current_user->ID;
    /* If user clicks to ignore the notice, add that to their user meta */
    if ( isset($_GET['sip_rswc_nag_ignore']) && '0' == $_GET['sip_rswc_nag_ignore'] ) {
        add_user_meta($user_id, 'sip_rswc_ignore_notice', 'true', true);
    }
  }
