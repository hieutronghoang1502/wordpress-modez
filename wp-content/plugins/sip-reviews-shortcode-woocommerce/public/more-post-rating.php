<?php
	...
	$query = "SELECT c.* FROM {$wpdb->prefix}posts p LEFT JOIN {$wpdb->prefix}comments c ON p.ID = c.comment_post_ID LEFT JOIN {$wpdb->prefix}commentmeta cm ON c.comment_ID = cm.comment_id WHERE p.ID = {$id} AND cm.meta_key = 'rating' AND cm.meta_value = {$number} AND c.comment_approved > 0 AND p.post_type = 'product' AND p.post_status = 'publish' AND p.comment_count > 0  AND c.comment_parent = 0 GROUP BY cm.comment_id ORDER BY c.comment_date DESC";
