<?php
	include_once('wd_menus_init.php');
	include_once('wd_menus_admin.php');
...
class WD_Walker_Nav_Menu extends Walker_Nav_Menu{
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$search_style = (int) get_post_meta( $item->ID, WD_MEGA_MENU.'_menu_item_search_style', true );
		...
		if($depth < 3){
			...
			if( (int) $item->sub_count > 0 || ( $this->parent_mega_enable == true && strlen($side_bar) > 0 ) ){
				...
				//$click_icon_html = "{$indent}\t\t<span class=\"visible-phone menu-drop-icon drop-icon-lv{$depth}\"></span>";
		...
		**/
		...
		if( $this->parent_mega_enable == true ){
			...
			if( $depth == 0 && $this->lv0_subcount == 0 ){
				...
				if( $this->search_style == 1  ){
					//$output .= "{$indent}". get_product_search_form(false) ."\n";
					$output .= "{$indent}". get_search_form(false) ."\n";
				}
