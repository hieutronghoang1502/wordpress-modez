...
function wd_mega_menu_script(){
		...
		wp_register_style( 'css_wd_menu_frontend', TVLGIAO_WPDANCE_THEME_CSS.'/wd_menu_front.css');
		wp_enqueue_style('css_wd_menu_frontend');		
		
		wp_enqueue_script( 'wd_some_handle',TVLGIAO_WPDANCE_THEME_JS.'/wd_menu_front.js' );
		$translation_array = array( 'wd_vertical_menu' => __( 'shop by departments','wpdancelaparis' ) );
		wp_localize_script( 'wd_some_handle', 'wd_object_name', $translation_array );
	
		//wp_register_script( 'js_wd_menu_frontend', TVLGIAO_WPDANCE_THEME_JS.'/wd_menu_front.js',false,false,true);
		//wp_enqueue_script('js_wd_menu_frontend');
		wp_register_script( 'jquery.hoverIntent', TVLGIAO_WPDANCE_THEME_JS.'/jquery.hoverIntent.js',false,false,true);
...
function add_support_scripts_backend(){
		...
		wp_register_style( 'css_wd_menu_backend', TVLGIAO_WPDANCE_THEME_CSS.'/wd_menu_back.css');
		wp_enqueue_style('css_wd_menu_backend');			
		
		wp_register_script( 'js_wd_menu_backend', TVLGIAO_WPDANCE_THEME_JS.'/wd_menu_back.js');
		...
		if($numSidebars > 0 ){
			if($numSidebars == 1){
				register_sidebar(array(
					'name'          => __('Wd Mega Menu Widget Area 1','wpdancelaparis'),
					'id'            => 'wd-mega-sidebar',
				...
				register_sidebars( $numSidebars, array(
					'name'          => __('Wd Mega Menu Widget Area %d','wpdancelaparis'),
					'id'            => "wd-mega-sidebar",
